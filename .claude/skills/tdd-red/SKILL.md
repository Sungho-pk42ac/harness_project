---
name: tdd-red
description: 승인된 테스트 시나리오(`docs/features/tag/issue-{N}.md`)를 "반드시 실패하는" Vitest 테스트 코드로 옮기고 RED를 확인하는 TDD RED 단계 스킬입니다. `/tdd-red {이슈번호}`로 부르거나, 사용자가 "RED 단계", "실패하는 테스트부터 작성", "시나리오를 테스트 코드로", "이슈 N TDD 시작", "테스트 먼저 짜자(test-first)" 등을 말하면 반드시 사용하세요. "RED"·"TDD"를 명시하지 않아도, 이미 도출된 시나리오/시그니처를 테스트 코드로 옮기려는 의도가 보이면 발동합니다. (구현 로직은 작성하지 않습니다 — 테스트와 최소 스텁, RED 확인까지만. 통과시키는 GREEN 단계는 별도.)
---

# TDD RED

`test-scenarios` 스킬이 만든 **`docs/features/tag/issue-{N}.md`**(상단=확정 시그니처, 하단=승인된 `should…when…` 시나리오)를 입력받아, **반드시 실패하는 Vitest 테스트 코드**로 옮기고 **실행해서 RED를 확인**합니다. 이후 테스트를 통과시키는 **GREEN(구현) 단계로 인계**합니다.
...
`docs/features/tag/issue-{N}.md`를 읽어 **확정 시그니처**와 **테스트 시나리오**(함수/컴포넌트별 그룹 + 분류 태그)를 파악합니다. AC 커버리지 표도 함께 봐 두면 ⑥ 보고에서 매핑할 수 있습니다.
...
> 이 프로젝트의 기능 폴더는 `tag`이고 테스트 대상은 `src/api`·`src/context`·`src/components`입니다. 다른 기능 이슈라면 `issue-{N}.md` 경로와 대상 소스 경로를 그 기능에 맞게 바꾸세요.
