...
> 태그 기능 정의서: [`docs/features/tag/spec-fixed.md`](docs/features/tag/spec-fixed.md) (구현 시 참고)
...
| --- | ------------ | -------------------------------- | -------------------------------------------------------------------------------- | ------------------ |
| 1   | **시나리오** | `test-scenarios` (skill)         | 시그니처(계약) 확정 + `should…when…` 시나리오 → `docs/features/tag/issue-{N}.md` | 시나리오 승인 후 ▶ |
